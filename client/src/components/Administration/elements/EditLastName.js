...
export default function EditLastName(props) {
  const classes = useStyles();
  const [values, setValues] = React.useState(props.lastname);
  ...
  return (
    <form className={classes.root} noValidate autoComplete="off">
      <FormControl className={classes.margin} error={props.error.length === 0 ? false : true }>
        <InputLabel shrink htmlFor="admin-edit-lastname-input">
          Last Name
        </InputLabel>
        <BootstrapInput
          id="lastname-text"
          ...
          defaultValue={props.lastname.lastname}
